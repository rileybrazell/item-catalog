...
# URLs contain category id, #
# used to get entry from Category table and commit changes to name #
...
# Each item is associated with a category id on creation #
...
# Sends item object to be deleted, category_id is for #
# redirecting to correct category afterwards #
...
def deleteItem(category_id, item_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction()\
        {alert('You are not authorized to delete items from this category.');}\
        </script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['confirm']:
